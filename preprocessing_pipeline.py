...
parser.add_argument('--path', type=str,
                    help='Path to the input JSONL file containing tweets')
...
def get_most_frequent_annotations(rdd, threshold):
    ...
    annotations = rdd.flatMap(lambda x: x['data']).map(lambda x: x['context_annotations']).filter(lambda x: x is not None).flatMap(lambda x: list(
        set([y['entity']['name'] for y in x]))).map(lambda x: (x, 1)).reduceByKey(lambda x, y: x+y).sortBy(lambda x: x[1], ascending=False)
...
def apply_processing_pipeline(json_rdd_data_fields, most_frequent_annotations, annotation_dict, output_name):
    ...
    json_rdd_data_fields = json_rdd_data_fields.map(lambda x: add_key_value(
    ...
    json_rdd_data_fields = json_rdd_data_fields.map(lambda x: {k: v for k, v in x.items(
    ) if k not in ['tweet_mentions', 'tweet_urls', 'tweet_hashtags', 'tweets_media_count']})
    ...
    json_rdd_data_fields = json_rdd_data_fields.map(
        lambda x: add_dummy_tweet_period(x))
    ...
    regression_df = json_rdd_data_fields.toDF().drop('context_annotations',
                                                     'encoded_annotations', 'tweet_date', 'tweet_media_keys', 'tweet_period').persist()
...
def pre_processing_pipeline(path, output_name, workers, annotation_threshold):
    ...
    regression_df = apply_processing_pipeline(
        rdd_subset, most_frequent_annotations, annotation_dict, output_name)
